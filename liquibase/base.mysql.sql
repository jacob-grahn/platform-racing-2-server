...
CREATE TABLE vault_items (slug VARCHAR(30) NOT NULL, active BIT DEFAULT 0 NOT NULL, placement TINYINT(3) DEFAULT 0 NOT NULL COMMENT 'In what order this item should show in the store.', title VARCHAR(30) DEFAULT '' NOT NULL, description VARCHAR(100) DEFAULT '' NOT NULL, faq VARCHAR(1000) DEFAULT '' NOT NULL, price SMALLINT DEFAULT 0 NOT NULL COMMENT 'Number of coins needed to purchase this item.', sale BIT DEFAULT 0 NOT NULL COMMENT 'Activating this will override global sale settings.', sale_value TINYINT(3) DEFAULT 0 NOT NULL COMMENT 'Value of sale percentage. Intended: int between 0 and 100.', sale_expires BIGINT DEFAULT 0 NOT NULL COMMENT 'Timestamp of sale expiry.', img_url VARCHAR(100) DEFAULT '' NOT NULL, img_url_small VARCHAR(100) DEFAULT '' NOT NULL, CONSTRAINT PK_VAULT_ITEMS PRIMARY KEY (slug), UNIQUE (placement))
