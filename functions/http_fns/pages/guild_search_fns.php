...
function output_guild_search($guild_name = '', $guild_id = '', $mode = null)
{
    ...
    $id_display = $mode === 'id' ? 'block' : 'none';
    $id_checked = $mode === 'id' ? 'checked="checked"' : '';
    $name_display = $mode === 'name' ? 'block' : 'none';
    $name_checked = $mode === 'name' ? 'checked="checked"' : '';

    // check if values passed are empty
    $guild_name = is_empty($guild_name) ? '' : $guild_name;
    $guild_id = is_empty($guild_id, false) ? '' : $guild_id;
