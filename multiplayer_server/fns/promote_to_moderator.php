...
function promote_mod($port, $name, $type, $admin, $promoted_player) {
	...
	if ($type == 'trial' || $type == 'permanent') {
		...
		try {
			...
			$db->call('mod_action_insert', array($admin->user_id, "$admin_name promoted $promoted_name to a $type moderator from $ip on $server_name", $admin->user_id, $ip));
